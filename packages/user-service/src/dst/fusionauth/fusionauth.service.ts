...
export class FusionauthService {
  ...
  constructor() {
    this.fusionauthClient = new FusionAuthClient(
      process.env.FUSIONAUTH_OLD_API_KEY,
      process.env.FUSIONAUTH_OLD_BASE_URL,
    );
  }
  ...
  getUsersByString(
    ...
    const searchRequest = {
      search: {
        numberOfResults: numberOfResults,
        query:
          `{"bool":{"must":[{"bool":{"must":[[{"nested":{"path":"registrations","query":{"bool":{"should":[{"match":{"registrations.applicationId":"${process.env.FUSIONAUTH_APPLICATION_ID}"}},{"match":{"registrations.applicationId":"${process.env.FUSIONAUTH_SAMARTH_HP_APPLICATION_ID}"}}]}}}}]]}},{"query_string":{"query":"${queryString}"}}]}}`,
        sortFields: [
          {
            missing: 'username',
            name: 'fullName',
